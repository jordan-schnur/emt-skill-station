...
function ReferenceSubTab() {
  ...
  const filtered = BLS_MEDICATIONS.filter((m) => {
    ...
    const matchesQuery = !q || m.name.toLowerCase().includes(q) || (m.genericName?.toLowerCase().includes(q) ?? false);
...
function ScenariosTab({ onBack }: { onBack: () => void }) {
  ...
  if (done || queue.length === 0) {
    ...
    return (
      <div class="blsmed-scenarios-done">
        ...
        <button class="btn btn-primary" type="button" onClick={onBack}>← Back to Reference</button>
      </div>
    );
  }
  ...
  function advance() {
    ...
    else { setIdx((i) => i + 1); setAnswered(false); setSelectedAnswer(null); setDeepMode(false); setFollowUpIdx(0); setFollowUpAnswered(false); setFollowUpSelected(null); }
  }
  ...
  function nextFollowUp() {
    if (followUpIdx + 1 >= scenario.followUps.length) { setDeepMode(false); advance(); }
    else { setFollowUpIdx((i) => i + 1); setFollowUpAnswered(false); setFollowUpSelected(null); }
  }
          ...
          <div class="blsmed-followup-card">
            ...
            <div class="blsmed-followup-options">
              {currentFollowUp.options.map((opt) => (
                <button key={opt}
                  class={`blsmed-option btn${followUpAnswered && opt === currentFollowUp.answer ? " correct" : followUpAnswered && opt === followUpSelected && opt !== currentFollowUp.answer ? " wrong" : ""}`}
                  type="button" disabled={followUpAnswered} onClick={() => chooseFollowUp(opt)}>{opt}</button>
              ))}
            </div>
            {followUpAnswered && <button class="btn btn-primary" type="button" onClick={nextFollowUp}>Next →</button>}
          </div>
        )
      )}
    </div>
...
const CARD_LABELS: Record<CardType, string> = { dose: "Dose", indications: "Indications", contraindications: "Contraindications", route: "Route" };
const CARD_QUESTIONS: Record<CardType, string> = { dose: "What is the adult dose?", indications: "When is it indicated?", contraindications: "When should you withhold it?", route: "How is it administered?" };
...
function buildDrillQueue(srsStore: Record<string, SRSRecord>) {
  ...
  for (const med of BLS_MEDICATIONS) {
    for (const type of CARD_TYPES) {
      ...
      all.push({ med, type, key, rec: srsStore[key] ?? defaultRecord() });
    }
  }
...
function DrillTab({ onBack }: { onBack: () => void }) {
  const srsStore = appState.value.blsMedsSrs ?? {};
  ...
  if (done || queue.length === 0) {
    return (
      <div class="blsmed-drill-done">
        ...
        <button class="btn" type="button" onClick={onBack}>← Back to Reference</button>
      </div>
    );
  }
  ...
  function applyGrade(g: "again" | "hard" | "good" | "easy") {
    ...
    mutateState((draft) => { if (!draft.blsMedsSrs) draft.blsMedsSrs = {}; draft.blsMedsSrs[key] = updated; });
    save();
    if (idx + 1 >= queue.length) setDone(true);
    else { setIdx((i) => i + 1); setRevealed(false); }
  }

  const answerItems = type === "indications" ? med.indications : type === "contraindications" ? med.contraindications : type === "route" ? med.route : null;
  const answerText = type === "dose" ? med.dose.adult + (med.dose.pediatric ? ` · Peds: ${med.dose.pediatric}` : "") : null;
      ...
      <div class="blsmed-drill-card">
        ...
        {revealed && (
          <div class="blsmed-drill-card-back">
            {answerItems ? <ul class="blsmed-list">{answerItems.map((item, i) => <li key={i}>{item}</li>)}</ul> : <div class="blsmed-drill-text-answer">{answerText}</div>}
          </div>
        )}
        {!revealed ? (
          ...
          <div class="blsmed-grade-row">
            {(["again", "hard", "good", "easy"] as const).map((g, i) => (
              <button key={g} class={`btn ${i === 0 ? "btn-danger" : i === 2 ? "btn-primary" : ""}`} type="button" onClick={() => applyGrade(g)}>
